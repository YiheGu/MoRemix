...
export function PldMotionPopUp(
  pld,
  pldMotion,
  allPldParams,
  oriallPldParams,
  ...
  if (mountTarget) {
    return createInlinePopup(pld, pldMotion, allPldParams, oriallPldParams, scene, mountTarget);
  }
  return createLegacyPopup(pld, allPldParams, scene);
}

function createInlinePopup(pld, pldMotion, allPldParams, oriallPldParams, scene, mountTarget) {
  const selector = `[data-popup-id="${pld.id}"]`;
  ...
  card.dataset.popupId = pld.id;

  const title = document.createElement("h3");
  title.innerText = `PLD ${pld.id} · ${pld.name}`;
  ...
  // angleImg.src = "";
  angleImg.src = pld.Ori.MPAnglePlot || "";
  ...
  // fftImg.src = "";
  fftImg.src = pld.Ori.MPSpectrumPlot || "";
  ...
  input.step = "any";
  input.min = "0";
  input.value = allPldParams[pld.id]?.MPAmp?.ampScale !== undefined ? allPldParams[pld.id].MPAmp.ampScale : 1;
  input.addEventListener("input", () => {
    allPldParams[pld.id].MPAmp.ampScale = parseFloat(input.value);
  });
  ...
  meanAddSlider.value = (allPldParams[pld.id]?.MPAmp?.meanAdd !== undefined ? allPldParams[pld.id].MPAmp.meanAdd : 0) * RAD_TO_DEG;
  ...
  meanAddSlider.addEventListener("input", () => {
    ...
    allPldParams[pld.id].MPAmp.meanAdd = valDeg * DEG_TO_RAD;
  ...
  keepFreqSelect.value = allPldParams[pld.id]?.MPAmp?.keepFreq || "Default";
  keepFreqSelect.addEventListener("change", () => {
    allPldParams[pld.id].MPAmp.keepFreq = keepFreqSelect.value;
  });
  ...
  v3Ratio.step = "any";
  v3Ratio.min = "0";
  v3Ratio.value = allPldParams[pld.id]?.V3Ratio !== undefined ? allPldParams[pld.id].V3Ratio : 1;
  v3Ratio.addEventListener("input", () => {
    allPldParams[pld.id].V3Ratio = parseFloat(v3Ratio.value);
  });
  ...
  v3MeanAddSlider.value = allPldParams[pld.id]?.V3MeanAdd !== undefined ? allPldParams[pld.id].V3MeanAdd : 0;
  ...
  v3MeanAddSlider.addEventListener("input", () => {
    ...
    allPldParams[pld.id].V3MeanAdd = val;
  ...
  phaseShiftSlider.value = allPldParams[pld.id]?.PhaseShift !== undefined ? allPldParams[pld.id].PhaseShift : 0;
  ...
  phaseShiftSlider.addEventListener("input", () => {
    ...
    allPldParams[pld.id].PhaseShift = val;
  ...
  generateBtn.addEventListener("click", () => {
    console.log("New pld motion clip shall be generated!");
  });

  const initializeBtn = document.createElement("button");
  initializeBtn.innerText = "Initialize";
  initializeBtn.addEventListener("click", () => {
    allPldParams = deepClone(oriallPldParams);
    console.log("PLD motion clip shall be initialized!");

    // Update UI to default values
    input.value = allPldParams[pld.id].MPAmp.ampScale;
    meanAddSlider.value = allPldParams[pld.id].MPAmp.meanAdd * RAD_TO_DEG;
    meanAddValueInput.value = meanAddSlider.value;
    keepFreqSelect.value = allPldParams[pld.id].MPAmp.keepFreq;
    v3Ratio.value = allPldParams[pld.id].V3Ratio;
    v3MeanAddSlider.value = allPldParams[pld.id].V3MeanAdd;
    v3MeanAddValueInput.value = v3MeanAddSlider.value;
    phaseShiftSlider.value = allPldParams[pld.id].PhaseShift;
    phaseShiftValueInput.value = phaseShiftSlider.value;
  });
  ...
  const api = {
    ...
    renew: () => {
      input.value = 1;
      allPldParams[pld.id].AmplitudeScale = 1;
      pcaSlider.value = 0.5;
      allPldParams[pld.id].pcaPosition = 0.5;
      thirdAxisSlider.value = 0.5;
      allPldParams[pld.id].thirdAxis = 0.5;
    },
