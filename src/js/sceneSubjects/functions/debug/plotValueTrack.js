...
export function plotValueTrack(valueTrack, newValueTrack, widthOrOptions = 420, height = 230, popupRef = null) {
  const {
    width,
  ...
  if ((!valueTrack || valueTrack.length === 0) && (!newValueTrack || newValueTrack.length === 0)) {
    const dataUrl = emptySvg(width, resolvedHeight);
    syncPopupImage(popup, titleKey, dataUrl);
    return dataUrl;
  }

  const originData = Array.isArray(valueTrack) ? valueTrack.map(toFiniteNumber) : [];
  const newData = Array.isArray(newValueTrack) ? newValueTrack.map(toFiniteNumber) : [];
  ...
  parts.push(`<defs><clipPath id="value-plot-clip"><rect x="${plotLeft}" y="${plotTop}" width="${plotWidth}" height="${plotHeight}"/></clipPath></defs>`);
  ...
  if (newPath) {
    parts.push(`<polyline points="${newPath}" fill="none" stroke="#d9364f" stroke-width="2" clip-path="url(#value-plot-clip)"/>`);
  }
  if (originPath) {
    parts.push(`<polyline points="${originPath}" fill="none" stroke="#2f6fe4" stroke-width="2" clip-path="url(#value-plot-clip)"/>`);
  }
  ...
  parts.push(`<g transform="translate(18 ${canvasHeight / 2}) rotate(-90)"><text x="0" y="0" text-anchor="middle" dominant-baseline="middle" fill="#1f2740" font-size="13" font-family="Segoe UI, sans-serif">${escapeXml(yLabel)}</text></g>`);
...
function appendMeanLine(parts, data, strokeColor, plotLeft, plotRight, plotTop, plotHeight, minVal, valueRange) {
  ...
  parts.push(`<text x="${plotRight - 2}" y="${y - 4}" text-anchor="end" fill="${strokeColor}" font-size="11" font-family="Segoe UI, sans-serif" opacity="0.9">${escapeXml(`mean=${mean.toFixed(3)}`)}</text>`);
}
...
function resolveConfig(widthOrOptions, height, popupRef, defaultWidth, defaultHeight) {
  if (widthOrOptions && typeof widthOrOptions === "object") {
    return {
      ...
      yLabel: String(widthOrOptions.yLabel || "Value"),
      titleKey: String(widthOrOptions.imageType || "v3"),
